          ...
          >
            ...
            >
              ...
              <h1
                className={
                  isNativeApp
                    ? "text-3xl font-bold leading-tight text-center px-2"
                    : "text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold leading-tight"
                }
              >
                Book <span className="text-accent">live talents</span> for your
                event
              </h1>

              <div
                className={`relative ${
                  isNativeApp ? "min-h-[3rem]" : "min-h-[4rem]"
                } flex items-center ${
                  isNativeApp ? "justify-center px-4" : "justify-start"
                }`}
              >
                <p
                  ...
                  className={`${
                    isNativeApp
                      ? "text-sm text-center text-muted-foreground/90 max-w-lg"
                      : "text-base sm:text-lg max-w-lg text-muted-foreground"
                  } leading-relaxed animate-fade-in`}
                  dir={biographyTexts[currentBiographyIndex].dir}
                  style={{
                    textAlign:
                      biographyTexts[currentBiographyIndex].dir === "rtl"
                        ? "right"
                        : "left",
                  }}
                >
              ...
              >
                <div className={`${isNativeApp ? "space-y-2" : "space-y-1.5"}`}>
                  <label
                    className={`${
                      isNativeApp
                        ? "text-[11px] font-medium text-muted-foreground block mb-2"
                        : "text-xs sm:text-sm font-semibold text-foreground uppercase tracking-wide"
                    }`}
                  >
                  ...
                  <div className="relative">
                    <MapPin
                      ...
                      } top-1/2 -translate-y-1/2 text-muted-foreground z-[11] pointer-events-none`}
                      style={{ left: isNativeApp ? "0.75rem" : "0.75rem" }}
                    />
                    <Select
                    ...
                    >
                      <SelectTrigger
                        className={`${
                          isNativeApp
                            ? "h-10 text-sm bg-background border-border/40"
                            : "h-11 sm:h-12 bg-background/50 border-border/50 hover:border-accent/50 text-sm sm:text-base"
                        } transition-colors w-full search-select-trigger`}
                        style={{
            ...
            >
              <div className="flex items-center space-x-2">
                ...
                <span className="text-muted-foreground text-center sm:text-left">
                  <strong className="text-foreground">4.9/5</strong> from 2,340+
                  bookings
                </span>
              </div>
              <div className="h-4 w-px bg-border hidden sm:block"></div>
              <div className="text-muted-foreground text-center sm:text-left">
                <strong className="text-foreground">500+</strong> professional
                artists
              </div>
            </div>
              ...
              >
                ...
                {!isNativeExperience && (
                  <div className="text-xs text-muted-foreground mt-1">
                    Looking for{" "}
                    {searchFilters.talentType !== "all" &&
                    searchFilters.talentType ? (
                      <span className="font-medium">
                        {searchFilters.talentType}s
                      </span>
                    ) : (
          ...
          >
            <div
              className={`text-center ${
                isNativeApp ? "space-y-2" : "space-y-2"
              }`}
            >
              <div className="flex items-center justify-center gap-2">
                ...
                <h3
                  className={`${
                    isNativeApp ? "text-lg" : "text-xl"
                  } font-bold text-foreground`}
                >
                  Featured Pro Artists
                </h3>
              </div>
              <p
                className={`${
              ...
              >
                Premium verified talents with unlimited bookings
              </p>
            </div>
        ...
        >
          <div className={`${isNativeApp ? "space-y-3" : "space-y-4"}`}>
            <div
              ...
              } bg-accent/10 rounded-full mb-4`}
            >
              <HelpCircle
                className={
                  isNativeApp ? "h-6 w-6 text-accent" : "h-8 w-8 text-accent"
                }
              />
            </div>
            <h2
            ...
            >
              Our expert team personally curates recommendations based on your
              event type, budget, and style preferences. Get matched with
              verified performers in 24 hours.
            </p>
          </div>
            ...
            <Button
              ...
              onClick={() => {
                if (user) {
                  ...
                  navigate("/auth", {
                    state: { intent: "event-form", mode: "booker" },
                  });
                }
              }}
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 max-w-4xl mx-auto w-full px-4">
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Share your event details and preferences with us
                </p>
              </div>
              <div className="text-center space-y-2">
                <div className="inline-flex items-center justify-center p-2 bg-accent/10 rounded-full">
                ...
                <p className="text-sm text-muted-foreground">
                  Receive curated talent recommendations within 24h
                </p>
              </div>
              <div className="text-center space-y-2">
                <div className="inline-flex items-center justify-center p-2 bg-accent/10 rounded-full">
                ...
                <p className="text-sm text-muted-foreground">
                  Connect directly and create unforgettable experiences
                </p>
              </div>
            </div>
          )}
