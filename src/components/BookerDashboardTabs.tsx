...
export const BookerDashboardTabs = ({ userId }: { userId: string }) => {
  ...
  const fetchInitialData = useCallback(async () => {
    ...
    if (requestsResult.error) {
      console.error("Error fetching requests:", requestsResult.error.message);
    } else {
        ...
        >
          <div className={cn(isNativeApp ? "space-y-3 w-full" : "space-y-4")}>
            {directBookings.length > 0 ? (
              ...
              >
                You have not made any direct bookings.
              </p>
            )}
          </div>
          {hasMoreBookings && (
        ...
        >
          <div className={cn(isNativeApp ? "space-y-3 w-full" : "space-y-4")}>
            {eventRequests.length > 0 ? (
              ...
              >
                You have not made any event requests to our team.
              </p>
            )}
          </div>
          {hasMoreRequests && (
