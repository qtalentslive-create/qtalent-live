...
export const AdminNotificationEmail = ({
  ...
  const getMessage = () => {
    switch (notificationType) {
      case "new_booking":
        return `A new direct booking has been created by ${bookerName} for a ${eventType} event${
          eventDate ? ` on ${eventDate}` : ""
        }${eventLocation ? ` at ${eventLocation}` : ""}.`;
      case "new_event_request":
        return `A new event request has been submitted by ${bookerName} for a ${eventType} event${
          eventDate ? ` on ${eventDate}` : ""
        }${
          eventLocation ? ` at ${eventLocation}` : ""
        }. This is an indirect request that needs talent matching.`;
      case "payment_completed":
    ...
    <Html>
      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <div style={detailsBox}>
            ...
            {bookerName && (
              <Text style={detailsItem}>
                <strong>Client Name:</strong> {bookerName}
              </Text>
            )}
            {bookerEmail && (
              <Text style={detailsItem}>
                <strong>Client Email:</strong> {bookerEmail}
              </Text>
            )}
            {requesterEmail && (
              <Text style={detailsItem}>
                <strong>User Account Email:</strong> {requesterEmail}
              </Text>
            )}
            {bookerUserId && (
              <Text style={detailsItem}>
                <strong>User ID:</strong> {bookerUserId}
              </Text>
            )}
            {requestType && (
              <Text style={detailsItem}>
            ...
            {talentName && (
              <>
                ...
                {talentUserId && (
                  <Text style={detailsItem}>
                    <strong>Talent User ID:</strong> {talentUserId}
                  </Text>
                )}
              </>
            )}
            ...
            {(amount || bookingId || eventRequestId) && (
              <>
                ...
                {eventRequestId && (
                  <Text style={detailsItem}>
                    <strong>Event Request ID:</strong> {eventRequestId}
                  </Text>
                )}
                {talentId && (
                  <Text style={detailsItem}>
                    <strong>Talent Profile ID:</strong> {talentId}
                  </Text>
                )}
              </>
            )}
          ...
          <Text style={footer}>Qtalent.live Admin System</Text>
        </Container>
      </Body>
    </Html>
  );
