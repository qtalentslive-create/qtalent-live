-- Step 1: Set the service role key for trigger functions
INSERT INTO admin_settings (setting_key, setting_value, description) 
VALUES ('service_role_key', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im15eGl6dXBjY3dldWtyeGZkcW1jIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MjkyOTg4NCwiZXhwIjoyMDY4NTA1ODg0fQ.MzWB1P4vaSJWgtCj7BpdFvB0wz8p0x2dDMplPrtpFVY', 'Service role key for email notifications')
ON CONFLICT (setting_key) 
DO UPDATE SET 
  setting_value = EXCLUDED.setting_value,
  updated_at = now();

-- Step 2: Create comprehensive triggers for email notifications

-- Trigger function for new bookings (direct talent bookings)
...
BEGIN
  ...
  -- Send booking confirmation to booker
  BEGIN
    PERFORM net.http_post(
      url := 'https://myxizupccweukrxfdqmc.supabase.co/functions/v1/send-notification-email',
      headers := jsonb_build_object(
    ...
    RAISE LOG 'Booking confirmation sent to booker: %', NEW.user_id;
  EXCEPTION WHEN OTHERS THEN
    RAISE LOG 'Failed to send booking confirmation to booker: %', SQLERRM;
  END;
...
-- Trigger function for event requests (indirect bookings)
...
-- Step 3: Create all the triggers
...
CREATE TRIGGER booking_email_notification_trigger
  ...
  FOR EACH ROW EXECUTE FUNCTION public.send_booking_email_notification();

DROP TRIGGER IF EXISTS booking_status_email_notification_trigger ON public.bookings;  
CREATE TRIGGER booking_status_email_notification_trigger
  AFTER UPDATE ON public.bookings
  FOR EACH ROW EXECUTE FUNCTION public.send_booking_status_email_notification();

DROP TRIGGER IF EXISTS event_request_email_notification_trigger ON public.event_requests;
CREATE TRIGGER event_request_email_notification_trigger
  AFTER INSERT ON public.event_requests
  FOR EACH ROW EXECUTE FUNCTION public.send_event_request_email_notification();
...
CREATE TRIGGER message_email_notification_trigger
  ...
  FOR EACH ROW EXECUTE FUNCTION public.send_message_email_notification();

DROP TRIGGER IF EXISTS payment_email_notification_trigger ON public.payments;
CREATE TRIGGER payment_email_notification_trigger
  AFTER UPDATE ON public.payments
  FOR EACH ROW EXECUTE FUNCTION public.send_payment_email_notification();